...
class ObjexMaterialProperties(bpy.types.PropertyGroup):
    ...
    rendermode_blender_flag_ALPHA_CVG_SEL = bpy.props.BoolProperty(
            ...
            default=False # 421fixme does enabling this kill alpha?
        )
    ...
    geometrymode_G_FOG = bpy.props.EnumProperty(
            items=[
                ...
                ('AUTO','Auto','Set if blending uses Alpha (G_BL_CLR_FOG) or G_BL_A_FOG, clear otherwise',3), # I am only assuming this is good practice
            ],
            name='G_FOG',
...
for c in (0,1):
    ...
    # variable   choices                                                                    default
        ('P', ('Input (CC/Blender)','Framebuffer Color','Blend Color','Fog Color'),         'Input (CC/Blender)'),
        ('A', ('Color Combiner Alpha','Alpha','Shade Alpha','0'),                           'Color Combiner Alpha'),
        ('M', ('Input (CC/Blender)','Framebuffer Color','Blend Color','Fog Color'),         'Framebuffer Color'),
        ('B', ('1 - A','Framebuffer Alpha','1','0'),                                        '1 - A')
    ):
