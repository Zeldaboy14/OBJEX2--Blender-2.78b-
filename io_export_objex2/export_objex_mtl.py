...
30     G_BL_CLR_IN    G_BL_A_IN      G_BL_CLR_MEM   G_BL_A_MEM   most OPA use this
                ...
                name_map = {
                    "Input (CC/Blender)": "G_BL_CLR_IN",
                    "Framebuffer Color": "G_BL_CLR_MEM",
                    "Blend Color": "G_BL_CLR_BL",
                    "Fog Color": "G_BL_CLR_FOG",

                    "Color Combiner Alpha": "G_BL_A_IN",
                    "Alpha": "G_BL_A_FOG",
                    "Shade Alpha": "G_BL_A_SHADE",
                    "0": "G_BL_0",

                    "1 - A": "G_BL_1MA",
                    "Framebuffer Alpha": "G_BL_A_MEM",
                    "1": "G_BL_1",
                    "0": "G_BL_0",
                }
                if rm_bl_c0 == 'CUSTOM':
                    #blendCycle0flags = (getattr(objex_data, 'rendermode_blending_cycle0_custom_%s' % v) for v in ('P','A','M','B'))
                    blendCycle0flags = (name_map.get(getattr(objex_data, 'rendermode_blending_cycle0_custom_%s' % v), "UNKNOWN" )for v in ('P','A','M','B'))
                else:
                ...
                if rm_bl_c1 == 'CUSTOM':
                    ...
                    blendCycle1flags = (name_map.get(getattr(objex_data, 'rendermode_blending_cycle1_custom_%s' % v), "UNKNOWN" )for v in ('P','A','M','B'))
                else:
